...
<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div class="flex-1 min-w-0">
            ...
            <p class="mt-2 text-gray-600">Manage all your products</p>
        </div>
    ...
    <div class="mb-6 bg-white shadow-sm rounded-lg border border-gray-200 p-4">
        <form method="GET" action="{{ route('vendor.products.index') }}" class="grid grid-cols-1 gap-4 sm:grid-cols-4">
            <!-- Search -->
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            <img src="{{ $product->primaryImageUrl ?? '/images/placeholder.png' }}" alt="{{ $product->name }}" class="h-12 w-12 rounded-md object-cover">
                            <div class="ml-4">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ \Carbon\Carbon::parse($product->createdAt)->format('M d, Y') }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex items-center justify-end space-x-2">
                            ...
                            <a href="{{ route('vendor.products.edit', $product->id) }}" class="text-blue-600 hover:text-blue-900" title="Edit">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                            </a>
                            <form method="POST" action="{{ route('vendor.products.destroy', $product->id) }}" onsubmit="return confirm('Are you sure you want to delete this product?');" class="inline">
                                @csrf
                                @method('DELETE')
    ...
    @else
    <!-- Empty State -->
    <div class="bg-white shadow-sm rounded-lg text border border-gray-200 py-12">
