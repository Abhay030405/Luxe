        ...
        <x-card>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-gray-900">{{ number_format($stats['total_vendors'] ?? 0) }}</p>
                    @if(($stats['pending_vendor_applications'] ?? 0) > 0)
                        <p class="mt-2 text-sm text-amber-600">{{ $stats['pending_vendor_applications'] }} pending</p>
                    @elseif(($stats['total_vendors'] ?? 0) > 0)
                        <p class="mt-2 text-sm text-green-600">{{ $stats['approved_vendors'] ?? 0 }} active</p>
                    @else
                        <p class="mt-2 text-sm text-gray-400">No vendors yet</p>
                    @endif
                </div>
            ...
            <x-card title="Recent Orders" padding="false">
                ...
                @else
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                        <x-link href="{{ route('admin.orders.index') }}">View all orders →</x-link>
                    </div>
                @endif
            </x-card>
        </div>
